      ...
      <aside class="calendar-side">
        <div class="mini-calendar">
          <div class="mini-header">
            <button type="button" class="icon-button" (click)="moveMini(-1)" aria-label="Mês anterior"><tp-icon name="chevron-left"></tp-icon></button>
            <strong>{{ monthName(miniMonth) }}</strong>
            <button type="button" class="icon-button" (click)="moveMini(1)" aria-label="Próximo mês"><tp-icon name="chevron-right"></tp-icon></button>
          </div>
          <div class="weekday-row"><span *ngFor="let day of weekdays">{{ day }}</span></div>
          <div class="mini-days"><button type="button" *ngFor="let day of miniDays" (click)="selectDate(day)" [class.other-month]="day.getMonth() !== miniMonth.getMonth()" [class.selected]="sameDate(day, selectedDate)" [class.today]="sameDate(day, todayDate)">{{ day.getDate() }}</button></div>
        </div>
        <div class="legend-panel"><h3>Legenda de status</h3><span><i class="dot success"></i>Concluída</span><span><i class="dot danger"></i>Concluída com erro</span><span><i class="dot info"></i>Pendente / em andamento</span></div>
        <form class="quick-filters" [formGroup]="filters" (change)="load()">
          <h3>Filtros rápidos</h3>
          <label>Função<select formControlName="functionId"><option value="">Todas</option><option *ngFor="let option of options.functions" [value]="option.id">{{ option.name }}</option></select></label>
          <label>Usuário<select formControlName="responsibleUserId"><option value="">Todos</option><option *ngFor="let option of options.users" [value]="option.id">{{ option.name }}</option></select></label>
          <label>Status<select formControlName="status"><option value="">Todos</option><option *ngFor="let status of options.statuses" [value]="status">{{ statusLabel(status) }}</option></select></label>
          <button type="button" class="secondary-button" (click)="clearFilters()"><tp-icon name="filter"></tp-icon>Limpar filtros</button>
        </form>
      </aside>
      ...
      <article [class]="modal === 'delete' ? 'confirm-modal occurrence-modal' : 'detail-modal occurrence-modal'" [attr.role]="modal === 'delete' ? 'alertdialog' : 'dialog'" aria-modal="true" [attr.aria-label]="modalTitle" (click)="$event.stopPropagation()">
        ...
        <ng-template #occurrenceModal>
          <ng-container *ngIf="selected">
            ...
            <ng-template #occurrenceDetails>
              <p class="eyebrow">Detalhes da ocorrência</p><div class="occurrence-title"><span class="task-symbol"><tp-icon name="task"></tp-icon></span><div><h2>{{ selected.task.name }}</h2><span class="status" [class]="'status ' + statusTone(selected.status)">{{ statusLabel(selected.status) }}</span></div></div>
              <dl class="detail-grid"><div><dt>Função</dt><dd>{{ selected.task.function?.name || 'Não informada' }}</dd></div><div><dt>Data prevista</dt><dd>{{ selected.scheduledDate | date:'dd/MM/yyyy' }}</dd></div><div><dt>Horário</dt><dd>{{ selected.scheduledTime || 'Não informado' }}</dd></div><div><dt>Responsável</dt><dd>{{ selected.responsibleUser?.name || selected.task.responsibleUser?.name || selected.task.responsiblePosition?.name || 'Não definido' }}</dd></div><div *ngIf="selected.executedByUser"><dt>Executor</dt><dd>{{ selected.executedByUser.name }}</dd></div></dl>
              <form *ngIf="modal === 'complete'" [formGroup]="executionForm" (ngSubmit)="complete()" class="execution-form"><h3>Conclusão</h3><label class="form-field"><span>Duração <b>*</b></span><input type="text" formControlName="duration" placeholder="hh:mm"><small>Informe horas e minutos; a API receberá minutos.</small></label><label class="form-field"><span>Resultado <b>*</b></span><select formControlName="result"><option value="SUCCESS">Sucesso</option><option value="PARTIAL">Parcial</option><option value="ERROR">Erro</option></select></label><label class="form-field"><span>Observações</span><textarea formControlName="notes" maxlength="1000" placeholder="Informe observações sobre a execução da tarefa…"></textarea></label></form>
              <form *ngIf="modal === 'reschedule'" [formGroup]="rescheduleForm" (ngSubmit)="reschedule()" class="execution-form"><h3>Reagendar</h3><label class="form-field"><span>Nova data <b>*</b></span><input type="date" formControlName="scheduledDate"></label><label class="form-field"><span>Novo horário</span><input type="time" formControlName="scheduledTime"></label></form>
              <p class="form-alert error" *ngIf="modalError">{{ modalError }}</p>
              <footer><button class="secondary-button" type="button" (click)="closeModal()">Fechar</button><button *ngIf="auth.isAdmin && modal === 'details'" type="button" class="danger-button" (click)="openDelete()">Excluir da agenda</button><ng-container *ngIf="selected.canOperate && (selected.status === 'PENDING' || selected.status === 'IN_PROGRESS')"><button *ngIf="modal === 'details' && selected.status === 'PENDING'" type="button" class="secondary-button" (click)="start()"><tp-icon name="play"></tp-icon>Iniciar</button><button *ngIf="modal === 'details'" type="button" class="secondary-button" (click)="modal = 'reschedule'"><tp-icon name="rotate"></tp-icon>Reagendar</button><button *ngIf="modal === 'details'" type="button" class="primary-button" (click)="modal = 'complete'"><tp-icon name="check"></tp-icon>Concluir</button><button *ngIf="modal === 'complete'" type="button" class="primary-button" [disabled]="acting" (click)="complete()">{{ acting ? 'Salvando…' : 'Salvar conclusão' }}</button><button *ngIf="modal === 'reschedule'" type="button" class="primary-button" [disabled]="acting" (click)="reschedule()">{{ acting ? 'Salvando…' : 'Salvar reagendamento' }}</button></ng-container></footer>
            </ng-template>
          </ng-container>
        </ng-template>
      </article>
...
export class CalendarComponent implements OnInit, OnDestroy {
  ...
  title = String(this.route.snapshot.data['title'] ?? 'Calendário');
  ...
  readonly fullWeekdays = ['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo'];
  readonly timeSlots = Array.from({ length: 96 }, (_, index) => { const minutes = index * 15; return minutes % 60 === 0 ? `${String(Math.floor(minutes / 60)).padStart(2, '0')}:00` : ''; });
  now = new Date();
  ...
  load(): void { const days = this.visibleDays; this.loading = true; this.error = ''; this.api.calendar({ from: this.dateKey(days[0]), to: this.dateKey(days[days.length - 1]), scope: this.scope, ...this.filters.getRawValue() }).subscribe({ next: (response) => { this.response = response; this.events = new Map(response.days.map((day) => [day.date, day.occurrences])); this.loading = false; }, error: () => { this.loading = false; this.error = 'Não foi possível carregar o calendário.'; } }); }
  ...
  private monthDays(date: Date): Date[] { const first = this.monday(this.firstOfMonth(date)); const last = this.endOfMonth(date); const end = this.addDays(last, (7 - ((last.getDay() + 6) % 7) - 1) % 7); const count = Math.ceil((end.getTime() - first.getTime()) / 86400000 + 1); return Array.from({ length: count < 35 ? 35 : count }, (_, index) => this.addDays(first, index)); }
