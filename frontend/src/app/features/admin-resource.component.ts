...
type FieldType = 'text' | 'email' | 'password' | 'textarea' | 'number' | 'date' | 'time' | 'select' | 'checkbox';
...
const definitions: Record<ResourceKey, Definition> = {
  ...
  periodicities: { title: 'Periodicidades', singular: 'periodicidade', endpoint: 'periodicities', icon: 'repeat', columns: [{ label: 'Nome', value: (r) => value(r, 'name') }, { label: 'Tipo', value: (r) => value(r, 'type') }, { label: 'Intervalo', value: (r) => value(r, 'interval') }, { label: 'Estado', value: (r) => value(r, 'active') }], fields: [{ key: 'name', label: 'Nome', type: 'text', required: true }, { key: 'type', label: 'Tipo', type: 'select', required: true, options: 'periodicityTypes' }, { key: 'interval', label: 'Intervalo', type: 'number', required: true }, { key: 'daysOfWeek', label: 'Dias da semana (1–7, separados por vírgula)', type: 'text', when: (v) => v['type'] === 'SPECIFIC_WEEKDAYS' }, { key: 'dayOfMonth', label: 'Dia do mês', type: 'number', when: (v) => ['MONTHLY', 'BIMONTHLY', 'QUARTERLY', 'SEMIANNUAL', 'ANNUAL', 'SPECIFIC_MONTH_DAY'].includes(String(v['type'])) }, { key: 'month', label: 'Mês (1–12)', type: 'number', when: (v) => v['type'] === 'ANNUAL' }, { key: 'nonexistentDayRule', label: 'Regra para data inexistente', type: 'select', required: true, options: 'nonexistentRules' }, { key: 'active', label: 'Ativo', type: 'checkbox' }] },
...
export class AdminResourceComponent implements OnInit {
  ...
  options: Record<string, NamedOption[]> = {
    ...
    periodicityTypes: ['DAILY','WEEKLY','BIWEEKLY','MONTHLY','BIMONTHLY','QUARTERLY','SEMIANNUAL','ANNUAL','SPECIFIC_WEEKDAYS','SPECIFIC_MONTH_DAY','FIRST_BUSINESS_DAY','LAST_BUSINESS_DAY','CUSTOM_INTERVAL'].map((id) => ({ id, name: id })),
